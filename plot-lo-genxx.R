...
data <- read.csv("lo-ppsn-costart-i2-genxx-l60-p1024.csv")
...
for (threads in c(2,4,6,8,10)) {
  this.w.e <- wilcox.test(data[data$type.threads==paste0("gen32-costart-i2",threads),]$Evaluations, data[data$type.threads==paste0("gen64-costart-i2",threads),]$Evaluations)
  wilcox.df.e <- rbind(wilcox.df.e,
                       data.frame(threads=threads,
                                  wilcoxon=this.w.e$p.value,
                                  significant= (this.w.e$p.value < 0.10)))
  this.w.t <- wilcox.test(data[data$type.threads==paste0("gen32-costart-i2",threads),]$Time, data[data$type.threads==paste0("gen64-costart-i2",threads),]$Time)
