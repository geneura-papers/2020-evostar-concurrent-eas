...
implement multi-population evolutionary algorithms, but it breaks open many degrees of % or multi-deme? - Mario
...
disturbance will not boost diversity; however, an intermediate
disturbance hypothesis will. We will test different ways of creating % hypothesis? - Mario
this intermediate disturbance by changing how different threads
operate or disturbing it in some other ways.  % or other ways to create it? - Mario I'm in dubt because this has a different meaning.
...
Diversity is essential in evolutionary algorithms, especially in
...
heuristically measure the effects of different types of disturbances, % can we enumerate a few? - Mario
...
run in the area of bioinspired algorithms. As a matter of fact, using % One "as a matter of fact" is ok. But two? - Mario
